...
// PS03 - 2.3 - uses a Sobel kernel to compute the horizontal and vertical
...
// PS03 - 2.4 - create a vector containing the normalized values in a 1D Gaussian filter
...
// PS03 - 2.4 - blur across the rows of an image
...
// PS03 - 2.5 - create a vector containing the normalized values in a 2D Gaussian filter
...
// PS03 - 2.5 - Blur an image with a full  full 2D rotationally symmetric Gaussian kernel
...
// PS03 - 2.6 - Use principles of seperabiltity to blur an image using 2 1D Gaussian Filters
...
// PS03 - 3.0 -  Denoise an image using bilateral filtering
Image bilateral(const Image &im, float sigmaRange, float sigmaDomain, float truncateDomain, bool clamp){

    float sigmaDomainsq = 2.0 * pow(sigmaDomain,2);
    float sigmaRangesq = 2.0 * pow(sigmaRange,2);
            ...
            for (int z=0; z<imFilter.channels(); z++) {
                ...
                for (int xFilter=0; xFilter<sizeFilt; xFilter++){
                    for (int yFilter=0; yFilter<sizeFilt; yFilter++){
                        ...
                        factorDomain = exp( - (pow(xFilter-offset,2) +  pow(yFilter-offset,2) )/ sigmaDomainsq);
                        factorRange = exp( - factorRangeExp / sigmaRangesq);
...
// PS03 - 3.1 - 6.865 only: Bilaterial Filter an image seperatly for
